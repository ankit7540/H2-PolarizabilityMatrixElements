	...
	real*8 alphapar(numfreq,numdist),alphaperp(numfreq,numdist)
	real*8 alpha(numfreq,numdist),alpha0(numfreq),alpha2(numfreq)
	real*8 omval(numdist),omval2(numdist)
	character*2 elname(3),molname
	character*16 filename,omega,unitname 
	data elname /'H2','D2','HD'/

C	read laser frequency values (in Hartree) and transform to nm
C	############################################################
	open(20,file="data/freq.txt")
	do i=1,numfreq
	  read(20,*)r
	freq(i)=1.0d7/r/219474.6313702d0
	end do
	close(20)
...
C	read perpendicular polarizability values
C	########################################
	open(20,file="data/matrix_xxf.txt")
	do i=1,numfreq
	  read(20,*)(alphaperp(i,j),j=1,numdist)
	end do
	close(20)

C	read parallel polarizability values
C	###################################
	open(20,file="data/matrix_zzf.txt")
	do i=1,numfreq
	  read(20,*)(alphapar(i,j),j=1,numdist)
	end do
	close(20)
...
C	#################################
     ...
     *            ' v_ket, J_ket, laser wavelength lambda,',
     *            ' wavelength unit, Omega'
	write(*,*)'(example: H2 1 0 2 3 532 nm alpha_mean)'
	write(*,*)'   '
	...
	write(*,*)'   '
	read(*,*)molname,brav,braJ,ketv,ketJ,lambda,unitname,omega
...
C	####################################
	...
	if (trim(unitname) .eq. 'nm') then
	  if (lambda .lt. 182.26d0 .or. lambda .gt. 1320.0d0) then
	    print*,'error exit: lambda value out of valid interval'
	    stop
	  end if
	else if (trim(unitname) .eq. 'A') then
	  if (lambda .lt. 1822.6d0 .or. lambda .gt. 13200.0d0) then
	    print*,'error exit: lambda value out of valid interval'
	    stop
	  end if
	else if (trim(unitname) .eq. 'Hartree') then
	  if (lambda .lt. 0.0345 .or. lambda .gt. 0.25) then
	    print*,'error exit: lambda value out of valid interval'
	    stop
	  end if
	else
	  print*,'error exit: wrong laser wavelength unit name',
     *    'A'//trim(unitname)//'A'
	  stop
	end if

C	transform lambda units to nm
C	############################
	if (trim(unitname) .eq. 'A') then
	  lambda=lambda/10.0d0
	else if (trim(unitname) .eq. 'Hartree') then
	  lambda=1.0d7/lambda/219474.6313702d0
	end if
...
C	#############################################
	...
	do i=1,numfreq
	  do j=1,numdist
	    alpha(i,j)=alphapar(i,j)
	  end do
	end do
	else if (trim(omega) .eq. 'alpha_perp') then 
	do i=1,numfreq
	  do j=1,numdist
	    alpha(i,j)=alphaperp(i,j)
	  end do
	end do
	else if (trim(omega) .eq. 'alpha_mean') then 
	do i=1,numfreq
	  do j=1,numdist
	    alpha(i,j)=(2*alphaperp(i,j)+alphapar(i,j))/3.0d0
	  end do
	end do
	else if (trim(omega) .eq. 'gamma') then
	do i=1,numfreq
	  do j=1,numdist
	    alpha(i,j)=alphapar(i,j)-alphaperp(i,j)
	  end do
	end do
	end if
...
C	###################################################
	do j=1,numdist
	  ...
	  call deriv1a(freq,alpha0,der1)
	  call deriv1b(freq(numfreq-3),alpha0(numfreq-3),der2)
	  call spline(freq,alpha0,numfreq,der1,der2,alpha2)
	  call splint(freq,alpha0,alpha2,numfreq,lambda,omval(j))
	end do

C	find the spline parameters for omega at lambda
C	##############################################
	call deriv1a(dist,omval,der1)
	call deriv1b(dist(numdist-3),omval(numdist-3),der2)
	call spline(dist,omval,numdist,der1,der2,omval2)
...
C	#################################################
	...
	do l=1,ngrid
	  call splint(dist,omval,omval2,numdist,gridval(l),grand(l))
