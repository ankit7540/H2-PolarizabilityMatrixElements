...
if not(alpha_xx.shape ==  alpha_zz.shape  or len(omega)==alpha_xx.shape[0] ):
    ...
    omega_A=(omega_nm*10)
    print("Available wavelength range: ",omega[0],"-",omega[-1],"Hartree; ",omega_nm[0],"-",omega_nm[-1]," nm; ",omega_A[0],"-",omega_A[-1]," Angstrom.")
    print("...ready.")
#********************************************************************
...
def MEcompute(mol, vl, Jl, vr, Jr, wavelength, wavelength_unit, operator):
    ...
    #print("Param shape =",param.shape," omega_nm shape", omega_nm.shape) 
    interpolate2Dw(param,omega_nm,omegaFinal)
   
    parameter=np.zeros((len(distance),1))
    parameter[:,0]=output[:,0] 
    #print("shape of parameter and distance : ",parameter.shape , distance.shape)
    ...
    print("<psi1|parameter|psi2> = ",result[0])
